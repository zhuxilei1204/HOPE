...
WINDOW="${TMUX_WINDOW:-clv3V6BLifecycle}"
...
RUN_NAME="${RUN_NAME:-CLV3_SCRATCH_V6B_LIFECYCLE_MULTI114_${NUM_ENVS}x${MAX_ITERATIONS}_SEED${SEED}_${TAG}}"
...
task=HOPEPingPongClosedLoopV3ScratchLifecycleMotionV6BMultiSkill114
ab_role=B_only_adds_motion_lifecycle_sampling_to_A
paired_seed=738
...
ready_credit=signed_non_farmable_potential_weight_3
stage1_no_command_hold_probability=0.45
impact_health_floor_progress=targeted_attempt_ema_at_0.05
motion_seed_modes=prestrike_0.45,recovery_0.35,rsi_0.20
motion_recovery_lag_steps=8_to_30_after_annotated_strike
...
seed=${SEED}
device=${DEVICE}
promotion_gate=deterministic_no_ball_survival,healthy_contact,net_cross,both_sides,recovery,fall_and_action_limits
EOF
...
sha256sum \
  ...
  "${ROOT}/cfg/task/HOPEPingPongClosedLoopV3ScratchSafetyBootstrapV6AMultiSkill114.yaml" \
  "${ROOT}/cfg/task/HOPEPingPongClosedLoopV3ScratchLifecycleMotionV6BMultiSkill114.yaml" \
