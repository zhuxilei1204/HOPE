...
WINDOW="${TMUX_WINDOW:-clv3V6DBalReady}"
...
RUN_NAME="${RUN_NAME:-CLV3_SCRATCH_V6D_ADAPTIVE_BALANCED_READY_MULTI114_${NUM_ENVS}x${MAX_ITERATIONS}_SEED${SEED}_${TAG}}"
...
task=HOPEPingPongClosedLoopV3ScratchAdaptiveBalancedReadyV6DMultiSkill114
ab_role=C_plus_dedicated_no_command_active_balance_skill
paired_seed=738
...
stand_episode_probability=0.25
...
seed=${SEED}
device=${DEVICE}
promotion_gate=strict_no_command_survival,healthy_contact,net_cross,both_sides,recovery,fall_and_action_limits
EOF
...
sha256sum \
  ...
  "${ROOT}/cfg/task/HOPEPingPongClosedLoopV3ScratchAdaptiveBalancedReadyV6DMultiSkill114.yaml" \
