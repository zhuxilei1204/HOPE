...
class PlannerConfig:
    ...
    # Optional per-axis overrides. None preserves legacy callers that only set
    # poly_order; the deployed node explicitly selects xy=1 and z=2.
    poly_order_xy: Optional[int] = None
    poly_order_z: Optional[int] = None
    collect_fit_diagnostics: bool = False  # evaluator-only covariance/residual metrics
    fit_window_s: float = 0.14    # rate-independent short history; validation-selected
    fit_window: int = 67          # hard sample-count cap for abnormal/high-rate streams;
                                  # set fit_window_s <= 0 to use this legacy limit alone.
    min_ready_samples: int = 20   # real measurements required after startup or a bounce;
                                  # no physics-generated post-bounce state is published.
    mocap_hz: float = 300.0       # documentation-only nominal capture rate; fit_window_s
                                  # makes the estimator independent of actual stream rate.
