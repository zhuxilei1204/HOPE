...
def _lead_bins(rows: List[dict]) -> List[dict]:
    edges = [(0.0, 0.05), (0.05, 0.10), (0.10, 0.20), (0.20, 0.35), (0.35, float("inf"))]
    out = []
    for lo, hi in edges:
        selected = [r for r in rows if lo <= r["lead_time_s"] < hi]
        out.append({
            "lead_time": f"{int(lo*1000)}-{('inf' if math.isinf(hi) else int(hi*1000))} ms",
...
def evaluate_files(
    ...
    summary = {
        ...
        "planner": {
            ...
            "by_lead_time": _lead_bins(forecasts),
        },
        "configuration": {
            "x_hit_m": config.x_hit, "solve_period_s": solve_period_s,
            "fit_window_samples": config.fit_window, "split_y_m": split_y,
...
def _html_report(summary: dict, events: List[dict]) -> str:
    ...
    bins = "".join(
        f"<tr><td>{b['lead_time']}</td><td>{b['forecasts']}</td>"
        f"<td>{_fmt(b['intercept_error_m']['median'],1000,' mm')}</td>"
        f"<td>{_fmt(b['absolute_timing_error_s']['median'],1000,' ms')}</td></tr>"
        for b in p["by_lead_time"]
    )
...
<h2>按剩余反应时间分解</h2><table><tr><th>真值剩余时间</th><th>预测数</th><th>截点误差 P50</th><th>时间误差 P50</th></tr>{bins}</table>
...
def main(argv=None) -> None:
    ...
    ap.add_argument("--fit-window", type=int, default=31, help="Estimator fit window in samples.")
    ...
    config = PlannerConfig(
        x_hit=args.x_hit, fit_window=args.fit_window, C_r=paddle["C_r"],
